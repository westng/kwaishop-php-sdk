...
use Kwaishop\PhpSdk\Tests\Support\TestConfigFactory;

$usage = <<<'TEXT'
Kuaishou OAuth flow helper

Usage:
  php tests/manual/oauth_flow.php authorize [--app-type=self|service-market] [redirect_uri] <scope_csv> [state]
  php tests/manual/oauth_flow.php exchange <code>
  php tests/manual/oauth_flow.php refresh <refresh_token>
  php tests/manual/oauth_flow.php client-token

Examples:
  php tests/manual/oauth_flow.php authorize --app-type=self https://example.com/callback merchant_order,merchant_item local-test
  ...
  php tests/manual/oauth_flow.php refresh YOUR_REFRESH_TOKEN
  php tests/manual/oauth_flow.php client-token
TEXT;
